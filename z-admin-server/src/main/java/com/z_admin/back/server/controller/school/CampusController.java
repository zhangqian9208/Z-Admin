package com.z_admin.back.server.controller.school;
...
import com.z_admin.back.server.service.school.CampusService;
...
public class CampusController {
    ...
    public R<Page> list(@RequestParam(value = "page", defaultValue = "1") Integer page,
        ...
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-查看校区列表异常");
            //打印错误日志
    ...
    public R<String> edit(@RequestBody Campus campus){
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-修改校区数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //出现异常，返回错误信息
    ...
    public R<String> delete(@RequestParam("ids") List<String> ids) {
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-删除校区数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //出现异常，返回错误信息
