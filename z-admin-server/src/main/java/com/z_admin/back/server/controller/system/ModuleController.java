...
public class ModuleController {
    ...
    public R<Page> list(@RequestParam(value = "page",defaultValue = "1")Integer page,
        ...
        try{
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-查看模块列表异常");
            //打印错误日志
    ...
    public R<String> addSave(@RequestBody Module module){
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-新增模块数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //出现异常，返回错误信息
    ...
    public R<String> delete(@RequestParam("ids") List<Long> ids) {
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-删除模块数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //写入错误数据
    ...
    public R<Module> findById(@RequestParam("id") Long id) {
        try{
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-查询单条模块数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
    ...
    public R<String> update(@RequestBody Module module){
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-修改模块数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //出现异常，返回错误信息
    ...
    public R<List<Module>> list(){
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-查询全部模块异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
