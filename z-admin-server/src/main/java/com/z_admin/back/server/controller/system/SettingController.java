...
public class SettingController {
    ...
    public R<Page> list(@RequestParam(value = "page",defaultValue = "1")Integer page,
        ...
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-查询设置列表异常");
            //打印错误日志
    ...
    public R<String> addSave(@RequestBody Setting setting) {
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-新增设置数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //出现异常，返回错误信息
    ...
    public R<String> delete(@RequestParam("ids") List<Long> ids) {
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-删除设置数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //出现异常，返回错误信息
    ...
    public R<Setting> findById(@RequestParam("id") Long id) {
        try{
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-查询单条设置数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //02.返回错误信息
    ...
    public R<String> update(@RequestBody Setting setting){
        try {
            ...
            //记录自定义异常日志
            LoggerUtils.saveLogAndUser(Thread.currentThread().getStackTrace()[1],exception, "后台-修改设置数据异常");
            //打印错误日志
            log.error("Z-Admin后台模板工程出现异常！异常信息为：" + exception);
        }
        //出现异常，返回错误信息
